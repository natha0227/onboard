    ...
    <div class="wrap">
      <form action="guardar_solicitud.php" class="formulario" name="formu" method="post" required="required">
        <div>
          ...
          <div class="input-group" style="padding: 17px;">
                <select class="form-select" aria-label="Default select example" id="id_asesor" name="id_asesor" required="required">
                <?php
                    // Incluir el archivo de conexión
                    include("../iniciar_sesion/conexion.php");
        
                    // Consulta SQL para obtener los usuarios
                    $sql = "SELECT  id, nombre FROM usuario u WHERE u.lvel = 2";
                    $result = $conn->query($sql);
                ...
                ?>
			      ...
			      <label class="label active" for="id_asesor">Asesor:</label>
          </div>
