    ...
    <table class="table table-bordered table-responsive">
      ...
      <tbody>
        <?php
          ...
          $sql_listar = "SELECT solicitud.*, CONCAT(usuario.nombre, ' ', usuario.apellido) AS nombre_completo, estado_solicitud.id AS 'id_e_s', estado_solicitud.nombre_estado, metodo.nombre FROM solicitud INNER JOIN estado_solicitud ON solicitud.estado_solicitud = estado_solicitud.id INNER JOIN metodo ON solicitud.metodo = metodo.id INNER JOIN usuario ON solicitud.id_usuario = usuario.id WHERE solicitud.estado_solicitud = '3' AND solicitud.id_asesor = {$_SESSION['id']}";
          ...
          if ($ejecutar_sql_listar->num_rows > 0) {
            ...
            while ($datos = mysqli_fetch_array($ejecutar_sql_listar)) {
              echo '<tr>
                    ...
                    <td>' . $datos['nombre'] . '</td>
                    <td><a href="editar_solicitud2.php?id=' . $datos["id"] . '" class="btn btn-warning">
