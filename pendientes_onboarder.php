    ...
    <table class="table table-bordered table-responsive">
      ...
      <tbody>
        <?php
          ...
          $sql_listar = "SELECT solicitud.*, CONCAT(usuario.nombre, ' ', usuario.apellido) AS nombre_completo, estado_solicitud.id AS 'id_e_s', estado_solicitud.nombre_estado, metodo.nombre FROM solicitud INNER JOIN estado_solicitud ON solicitud.estado_solicitud = estado_solicitud.id INNER JOIN metodo ON solicitud.metodo = metodo.id INNER JOIN usuario ON solicitud.id_usuario = usuario.id WHERE solicitud.id_usuario = $_SESSION[id] AND solicitud.estado_solicitud = 1";
